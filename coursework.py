...
class SmallWorldModularNet:
    ...
    def __init__(self, p, n=1000, C=8, m=1000):
        ...
        self._excitatory = list(range(int(n * 0.8))) # indices [0,800) = excitatory

        inhibitory_offset = len(self._excitatory)  # index of first inhibitory neuron
        self._inhibitory = [
            (i + inhibitory_offset) for i in range(int(n * 0.2))
        ]  # indices [800,1000) = inhibitory
        ...
        self._communities = np.zeros(
            (self._num_modules, neuro_per_module, neuro_per_module)
        )
        ...
        self._W = self._scale_connections(
            self._connections, inhibitory_offset=inhibitory_offset
        )
    ...
    def _set_excitatory(self, m, neuro_per_module):
        ...
        for module in range(self._num_modules):
            ...
            while set_edges < m:
                ...
                if (source_neuro != target_neuro) and (
                    (source_neuro, target_neuro) not in connections
                ):
    ...
    def _set_inhibitory(self, exc_to_inhib, neuro_per_module):
        ...
        for inhib in self._inhibitory:
            set_edges = 0
            connections = []
            source_module = np.random.randint(
                self._num_modules
            ...
            while set_edges < exc_to_inhib:
                ...
                if (excitatory_source, inhib) not in connections:
                    self._connections[excitatory_source, inhib] = np.random.uniform(
                        0, 1
                    )
    ...
    def _set_delays(self, inhibitory_offset):
        ...
        self._D[:inhibitory_offset, :inhibitory_offset] = np.random.randint(
            1, 20, (inhibitory_offset, inhibitory_offset)
        )
    ...
    def _set_params(self, inhibitory_offset):
        ...
        c[:inhibitory_offset] = -65 + (
            15 * (r[:inhibitory_offset] * r[:inhibitory_offset])
        )  # excitatory
        d = np.zeros(self._num_neurons)
        d[inhibitory_offset:] = 2  # inhibitory
        d[:inhibitory_offset] = 8 - (
            6 * (r[:inhibitory_offset] * r[:inhibitory_offset])
        )  # excitatory
    ...
    def assert_connections(self, neuro_per_module):
        ...
        for neuro in range(self._num_neurons):
            assert (
                self._connections[neuro, neuro] == 0
            ), f"Self connection for neuron {neuro}"
            if neuro in self._inhibitory:
                # Assert it connects to every neuron in the network
                assert (
                    np.count_nonzero(self._connections[neuro, :])
                    == self._num_neurons - 1
                ), f"Number of outgoing connections for neuron {neuro} not equal to {self._num_neurons - 1}, it is {np.count_nonzero(self._connections[neuro, :])}"
                # Assert it receives 4 incoming excitatory connections
                assert (
                    np.count_nonzero(self._connections[:inhib_offset, neuro]) == 4
                ), f"Number of incoming excitatory connections for neuron {neuro} not equal to 4"
                # Assert these connections come from the same module
                for community in range(self._num_modules):
                    count_connections_from_module = np.count_nonzero(
                        self._connections[
                            community
                            * neuro_per_module : (community + 1)
                            * neuro_per_module,
                            neuro,
                        ]
                    )
                    assert (
                        count_connections_from_module in (4,0)
                    ), f"Number of incoming excitatory connections for neuron {neuro} from module {community} not equal to 4"

        for module in range(self._num_modules):
            module_min = module * neuro_per_module
            module_max = (module + 1) * neuro_per_module
            # assert each module has exactly 1000 connections to other excitatory neurons
            assert (
                np.count_nonzero(
                    self._connections[module_min:module_max, :inhib_offset]
                )
                == intra_edges
            ), f"Number of intra connections for module {module} not equal to {self._num_neurons}, it is {np.count_nonzero(self._connections[module_min: module_max, :inhib_offset])}"

        print("All Network Connections are Valid.")
    ...
    def simulate_net(self, T=1000):
        ...
        for t in range(T):
            ...
            for index in fired_ids:
                self._fired[t, index] = 1
        
        self._simulation_done = True
        self._simulation_length = T

    def plot_weights(self, title, save=False):
        """
    ...
    def plot_raster(self, T=1000, save=False):
        """
        ...
        if self._simulation_done == False:
            raise Exception("Please run the simulation first.")
        
        if T > self._simulation_length:
            raise Exception("Simulation length is shorter than the requested duration.")
        
        inhibitory_offset = len(self._excitatory)  # to only plot the excitatory neurons
        neuron_events = []
        for neuron in range(self._fired.shape[1]):
            firing_times = np.where(self._fired[:, neuron] > 0)[
                0
            ]  # Time indices where neuron has fired
    ...
    def plot_rolling_mean_per_module(self, T=1000, window=50, shift=20, save=False):
        """
        ...
        if self._simulation_done == False:
            raise Exception("Please run the simulation first.")
        if T > self._simulation_length:
            raise Exception("Simulation length is smaller than the requested duration.")
        if T < window:
            raise Exception("Window size is larger than the simulation duration.")
        if T < shift:
            raise Exception("Shift size is larger than the simulation duration.")
        ...
        for module in range(self._num_modules):
            ...
            fired_module = np.sum(self._fired[:T, offset_start:offset_end], axis=1)
            rolling_mean = np.convolve(
                fired_module, np.ones(window) / window, mode="valid"
            )[::shift]
            # Plot rolling mean
            plt.plot(
                time_array[: len(rolling_mean)],
                rolling_mean,
                label=f"Module {module + 1}",
                linewidth=0.6,
            )
...
# Test the Small World Modular Network. Uncomment the following lines to run the test.
"""
if __name__ == "__main__":
    net = SmallWorldModularNet(p=0)
